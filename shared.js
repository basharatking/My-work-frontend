...
const BRAND      = _CFG.BRAND || "RunDocs";
...
function toast(msg, type="success", duration=3200){
  ...
  c.appendChild(t);
  setTimeout(() => { t.style.opacity="0"; t.style.transition="opacity .25s"; setTimeout(()=>t.remove(),260); }, duration);
}
...
function _initMegaMenu(){
  ...
  document.addEventListener("click", e => { if(open && !overlay.contains(e.target) && !btn.contains(e.target)) closeMenu(); });
...
function _renderChips(tid, files){
  ...
  el.innerHTML = files.map((f,i) => `
    <div class="file-chip">
      <svg class="fc-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
...
function showDownloadBtn(tid, blob, filename){
  ...
  btn.innerHTML = `<svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Download ${_esc(filename)}`;
...
async function loadSplitMeta(file){
  try {
    ...
    if(el){ el.textContent = `Total: ${n} pages`; el.style.display="inline"; }
...
const LOGO_SVG = `<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
  <rect x="2" y="1" width="9" height="12" rx="1.5" fill="white" fill-opacity=".9"/>
  <rect x="5" y="4" width="5" height="1" rx=".5" fill="#2563EB"/>
  <rect x="5" y="6.5" width="5" height="1" rx=".5" fill="#2563EB"/>
  <rect x="5" y="9" width="3" height="1" rx=".5" fill="#2563EB"/>
  <path d="M10 1L13 4H10V1Z" fill="white" fill-opacity=".5"/>
  <circle cx="12.5" cy="12.5" r="2.5" fill="#10B981"/>
  <path d="M11.5 12.5L12.2 13.2L13.5 12" stroke="white" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"/>
</svg>`;

/* ── Mega Menu Data ── */
const MEGA_TOOLS = [
  {
    cat: "Convert PDF", icon: "🔄", color: "#EFF6FF",
    tools: [
  ...
  },
  {
    ...
    tools: [
      ...
      ["add-page-numbers.html","🔢","Page Numbers"],
    ]
  },
  {
  ...
  {
    cat: "AI PDF Tools", icon: "✨", color: "#EFF6FF",
    tools: [
      ["ai-tools.html#ask","💬","Ask PDF"],
      ["ai-tools.html#summary","📋","Summarize PDF"],
      ["ai-tools.html#notes","📝","Generate Notes"],
      ["ai-tools.html#quiz","❓","Quiz Generator"],
      ["ai-tools.html#keypoints","💡","Key Points"],
      ["ai-tools.html#translate","🌍","Translate PDF"],
    ]
  },
  {
    cat: "Business & Office", icon: "💼", color: "#F0FDF4",
    tools: [
...
<div id="megaOverlay" class="mega-overlay">
  ...
  <div class="mega-footer">
    <span class="mega-footer-text">🔒 Files deleted after processing &nbsp;·&nbsp; All tools free to use</span>
    <a href="index.html#tools" class="btn btn-primary btn-sm">View All Tools →</a>
  </div>
</div>

<div class="drawer-overlay" id="navOverlay" onclick="toggleMenu()"></div>
<div class="mobile-nav" id="mobileNav">
  <div style="padding:.85rem 1rem .5rem;border-bottom:1px solid var(--border);margin-bottom:.4rem">
    <span style="font-size:16px;font-weight:700;color:var(--ink)">Run<span style="color:var(--brand)">Docs</span></span>
  </div>
...
function buildFooter(){
  ...
  <div class="footer-bottom">
    <p>© ${new Date().getFullYear()} RunDocs. Files are automatically deleted after processing.</p>
