...
/* ── Theme ── */
(function(){
  const s = localStorage.getItem("catchpdf-theme");
...
function toggleDark(){
  ...
  localStorage.setItem("catchpdf-theme", next);
  _setDarkIcon();
}
...
function showDownloadBtn(tid, blob, filename, meta={}){
  ...
  try {
    ...
    sessionStorage.setItem("catchpdf_dl_url", url);
    sessionStorage.setItem("catchpdf_dl_name", filename);
    sessionStorage.setItem("catchpdf_dl_size", _bytes(blob.size));
    sessionStorage.setItem("catchpdf_dl_tool", tid);
    sessionStorage.setItem("catchpdf_dl_meta", JSON.stringify(meta));
  } catch(_) {}
...
async function callAI(endpoint, fd, resultId){
  ...
  if(body) body.innerHTML=`<span style="color:var(--brand)">Processing…</span>`;
  try {
    const resp = await fetch(API_BASE+endpoint, {method:"POST", body:fd});
    if(!resp.ok){ const j=await resp.json().catch(()=>({})); throw new Error(j.detail||"Processing failed."); }
    const data = await resp.json();
    if(body) body.textContent = data.result||data.text||JSON.stringify(data);
    toast("Result ready!","info");
...
/* ── GA ── */
function _trackEvent(name, params={}){ if(typeof gtag!=="undefined") gtag("event", name, params); }
...
const NAV_LOGO_SVG = `<div class="nav-logo-mark">
  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path d="M4 2.5h8l4 4V17a.5.5 0 0 1-.5.5h-11A.5.5 0 0 1 4 17V2.5z" fill="white" fill-opacity=".9"/>
    <path d="M12 2.5V6.5h4" stroke="rgba(255,255,255,.6)" stroke-width="1" fill="none"/>
    <path d="M7 10h6M7 12.5h4.5" stroke="#1D4ED8" stroke-width="1.2" stroke-linecap="round"/>
    <circle cx="15.5" cy="15.5" r="3.5" fill="#0EA5E9"/>
    <path d="M14 15.5l1.2 1.2L17 14.2" stroke="white" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" fill="none"/>
  </svg>
</div>`;
...
function buildNav(){
  ...
  <a href="index.html" class="nav-logo">${NAV_LOGO_SVG}<span class="nav-logo-name">Catch<span>PDF</span></span></a>
  ...
  <div class="nav-right">
    <span class="nav-badge"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>Files auto-deleted</span>
    <button class="nav-btn-icon" id="darkBtn" onclick="toggleDark()" title="Toggle theme"></button>
    <a href="index.html#tools" class="nav-cta">Get Started →</a>
  </div>
</nav>
<div class="drawer-overlay" id="navOverlay" onclick="toggleMenu()"></div>
<div class="mobile-nav" id="mobileNav">
  <div class="mobile-nav-hdr"><span style="font-size:16px;font-weight:800;color:var(--ink)">Catch<span style="color:var(--brand)">PDF</span></span></div>
  ...
  <div class="mobile-nav-sep">Features</div>
...
function buildFooter(){
  ...
  <div class="footer-inner">
    <div class="footer-brand">
      <a href="index.html" class="nav-logo" style="filter:brightness(10)">
        ${NAV_LOGO_SVG}
        <span class="nav-logo-name" style="color:#fff;font-size:18px">Catch<span style="color:#60A5FA">PDF</span></span>
      </a>
      <p>Professional PDF tools for everyone. Secure, fast, and free.</p>
      <div style="display:flex;flex-direction:column;gap:6px;margin-top:1rem">
        ...
        <div class="footer-badge"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>256-bit encryption</div>
      </div>
    </div>
    ...
    <div class="footer-col">
      <h4>Smart Tools</h4>
      <a href="ai-tools.html#summary">Summarize PDF</a><a href="ai-tools.html#notes">Generate Notes</a>
      <a href="ai-tools.html#quiz">Create Quiz</a><a href="ai-tools.html#translate">Translate PDF</a>
  ...
  <div class="footer-bottom">
    <p>© ${new Date().getFullYear()} CatchPDF. All rights reserved.</p>
